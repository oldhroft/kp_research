...
class TestYamlLoad:
    def test_simple_yaml_load(self):
        data = load_yaml(os.path.join(LOCATION, "test_yamls/test1.yaml"))
        assert data["test"]["value1"] == 1, "Yaml file is not correct"

    def test_include_yaml(self):
        data = load_yaml(os.path.join(LOCATION, "test_yamls/test2.yaml"))
        assert (
    ...
    def test_environ_yaml(self):
        ...
        data = load_yaml(os.path.join(LOCATION, "test_yamls/test_env.yaml"))
