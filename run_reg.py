...
from run_utils import (create_folder_structure, fit_reg, get_data_pipeline,
                       read_data, save_model, save_vars, score_reg, check_config)
...
if __name__ == '__main__':
    ...
    df_train, df_test, categories = read_data(regression=True, **config_global["data"])
    ...
    for model_name, config in config_global['models'].items():
        ...
        model = fit_reg(model_name, config['init_params'], X_train, y_train)

        logger.info(f'Scoring model, {model_name}')
        score_reg(model, model_name, X_test, y_test, structure, PROC_NAME)
